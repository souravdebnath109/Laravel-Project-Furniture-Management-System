        ...
        <div class="row">
            <div class="col-md-4 order-md-2 mb-4">
                <h4 class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <span class="badge badge-secondary badge-pill">{{$getCart->count()}}</span>
                </h4>
                <ul class="list-group mb-3">
                    @foreach ($getCart as $item)
                    <li class="list-group-item d-flex justify-content-between lh-condensed">
                        <div>
                            <h6 class="my-0">{{$getFurniture->find($item->furniture_id)->title}}</h6>
                        </div>
                        <span class="text-muted">{{$getFurniture->find($item->furniture_id)->price }}Tk X{{$item->quantity}}</span>
                    </li>
                    @endforeach
                ...
                <form action="{{ url('/pay') }}" method="POST" class="needs-validation">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="email" name="customer_email" class="form-control" id="email"
                            placeholder="you@example.com" value="{{Auth::user()->email}}" readonly required>
                        <div class="invalid-feedback">
